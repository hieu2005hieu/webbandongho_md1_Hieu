...
function renderProductByCategory(htmlId) {
    const products = JSON.parse(localStorage.getItem("listProduct")) || []
    let data
    if (htmlId == "watch-male") {
        data = products.filter(product => product.category == "Đồng hồ nam")
    } else {
        data = products.filter(product => product.category == "Đồng hồ nữ")
    }
    let stringHTML = ""
    for (let i = 0; i < data.length; i++) {
        stringHTML +=
            `
            <div class="title-img" onclick="gotoProductDetail(${data[i].ID})">
                <div class="frame-img">
                    <img class="img-product" src="${data[i].img}" alt="anh">
                  </div>
                <h5>${data[i].ten}</h5>
                <p>${Number(data[i].gia).toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}</p>
            </div>
            `
    }
    document.getElementById(htmlId).innerHTML = stringHTML
}
renderProductByCategory("watch-male")
renderProductByCategory("watch-female")
